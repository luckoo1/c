...
2. ★배열을 선언 할 떄는, Data Type 사이에 배열의 이름(변수)이 나온다
    int x[3] = {1, 2, 3}; //Data Type : int [3]

    만약에 C언어를 이렇게 만들었다면?
    int[3] x = {1, 2, 3}; 
    1번규칙(변수 선언시 Data Type 뒤에 변수 이름이 나온다)을 만족하고 일관성 있을텐데
    C는 이게 안된다
    요즘에 새로 나온 언어들은 이렇게 초기화 하는 경우도 있다.

    함수 만들때도 "int[3] f2()"하면 좋을텐데 C는 그게 안된다.

3."int f2()[3]"와 같이 적으면 3개짜리 배열을 반환하는 함수
*/
...
int a;
int foo();
a자리에 "함수이름이랑 괄호"(foo())를 넣었다.

int x[3];
int foo()[3];
x자리에 "함수이름이랑 괄호"(foo())를 넣었다.
