...
int main(void)
{
    cnt = 10;//cnt를 10으로 만들고 싶어서 추가했다.
    //gcc main.c goo.c하면 error발생
    //cnt를 찾을 수 없다는 error발생
    goo();
}
//-----------------------------------------
...
extern int cnt; //cnt는 다른파일에 있어서 링커가 찾아줄 것이라고 알려주는 것
int main(void)
{
    cnt = 10; //OK
    goo();
...
static void hoo();//2. 여기에 선언부를 선언해도
"goo.c"---------------------------------
int cnt = 0;
void goo(){ }

static void hoo(){} //1. 이 함수는 goo.c에서만 사용할수 있다.
static int cnt = 0;
...
그냥 "goo.h"에서 연결해서 main.c에 선언이 되어서 사용하는거다.
매크로도 내부연결성을 가지는 것으로 이해하자.
*/
