...
그림필기
"int x[3]"의 Data type는 "int[3]" 
 int *p2 = x; 에서
 x는 10의 주소로 암시적 형변환된다.
*/
...
int main(void)
{
    ...
    */
    ...
    int *p3 = *p1;
    //*(p1)
    //*(배열을 가리키는 포인터) => 배열자체를 가리킴 =>배열은 "첫번째 요소의주소"로 암시적 형변환됨
    //그래서 p3는 포인터변수(int*)로 받은거다.
    /*
    p1이 가리키는 곳에 가면 배열자체를 가리킴
    배열은 첫번째 요소인 10의 주소로 암시적 형변환됨
    10의 주소를 받기 위해 int* p3로 받음
    */
    int n = *p2;
    //*(int*) => int, 10
    /*
