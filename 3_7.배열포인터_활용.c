...
int main(void)
{
    ...
    //malloc은 데이터 type에 대한 정보를 받지않고 크기만 받음
    char *p1 = malloc(160);
    //명시적 캐스팅해서 아래처럼 쓰자
    ...
    // 2차원배열의 이름 == 1차 배열을 가리키는 포인터를 의미
    // 2차원 배열처럼 쓰고 싶으니 (int (*p3)[4])적었다.
    int(*p3)[4] = malloc(160);
    //이제 명시적 캐스팅 해야한다.
    int(*p3)[4] = (int(*p3)[4])malloc(160);
    //타입만 적어야하니깐 p3를 지우자
...
N차 배열의 이름을 "N-1차 배열을 가리키는 포인터와 같은 의미"이다 => "int(*p3)[4]" 이렇게 쓰자.
malloc이 반환한 주소를 "N-1차 배열을 가리키는 포인터 타입으로 캐스팅"한 후 사용하면 된다. =>"(int(*)[4])malloc(160);" 이렇게쓰자.
그래서 int(*p3)[4] = (int(*)[4])malloc(160); 이렇게 쓰이는 것
*/
